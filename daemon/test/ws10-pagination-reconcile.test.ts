...
describe("clampPage", async () => {
  it("default 50 when limit absent", async () => {
    expect(clampPage({})).toEqual({ limit: 50, offset: 0 });
  });

  it("clamps limit>200 to 200 (MCP hard cap, clamp-not-reject)", async () => {
    expect(clampPage({ limit: 500 })).toEqual({ limit: 200, offset: 0 });
  });

  it("clamps limit=0 to 50 (default)", async () => {
    expect(clampPage({ limit: 0 })).toEqual({ limit: 50, offset: 0 });
  });

  it("clamps limit=-1 (negative) to 50 (default)", async () => {
    expect(clampPage({ limit: -1 })).toEqual({ limit: 50, offset: 0 });
  });

  it("passes limit=25 through unchanged", async () => {
...
describe("listPendingPage", async () => {
  let baseDir: string;
  ...
  it("offset=0 limit=25: returns 25 items, total=60, has_more=true", async () => {
  ...
  it("offset=50: returns 10 items, has_more=false", async () => {
  ...
  it("limit=200 covers all 60, has_more=false", async () => {
    const page = engine.listPendingPage({ limit: 200, offset: 0 });
    expect(page.items.length).toBe(60);
    expect(page.has_more).toBe(false);
  });

  it("MCP-style clamp: limit=500 clamped to 200", async () => {
  ...
  it("default limit=50 when no opts passed", async () => {
  ...
  it("pages cover all items without overlap", async () => {
...
describe("listResourcesPage", async () => {
  let baseDir: string;
  let engine: EvolutionEngine;
  let sql: SqliteStore;
  ...
  it("offset=0 limit=25: 25 items, total=70, has_more=true", async () => {
  ...
  it("offset=60 limit=25: 10 items, has_more=false", async () => {
    const page = engine.listResourcesPage({}, { limit: 25, offset: 60 });
    expect(page.items.length).toBe(10);
    expect(page.has_more).toBe(false);
  });

  it("limit clamped to 200 (clamp-not-reject)", async () => {
...
describe("detectDriftPage", async () => {
  let baseDir: string;
  let engine: EvolutionEngine;
  let sql: SqliteStore;
  ...
  it("total=drift entries (30), total_resources=40, has_more with limit=10", async () => {
    const page = await engine.detectDriftPage({ limit: 10, offset: 0 });
  ...
  it("offset=25 limit=10: 5 items left, has_more=false", async () => {
    const page = await engine.detectDriftPage({ limit: 10, offset: 25 });
    expect(page.items.length).toBe(5);
    expect(page.has_more).toBe(false);
  });

  it("limit clamped to 200 (clamp-not-reject), returns all 30 drifts", async () => {
    const page = await engine.detectDriftPage({ limit: 999 });
    expect(page.limit).toBe(200);
    expect(page.items.length).toBe(30); // only 30 drift entries
    expect(page.has_more).toBe(false);
  });

  it("FIX 3: resource with N sources yields N drift entries, all subject to limit", async () => {
    const env = makeEnv();
    ...
    const page = await engine.detectDriftPage({ limit: 3, offset: 0 });
    expect(page.items.length).toBe(3);
    expect(page.total).toBe(35);
    expect(page.has_more).toBe(true);
    // Confirm no single call exceeds the limit.
    for (let off = 0; off <= page.total; off += 3) {
      const p = await engine.detectDriftPage({ limit: 3, offset: off });
      expect(p.items.length).toBeLessThanOrEqual(3);
    }
  });

  it("total_registry + total_sources present on every page regardless of limit (single-call status pattern)", async () => {
    ...
    const p1 = await engine.detectDriftPage({ limit: 1, offset: 0 });
    ...
    const p2 = await engine.detectDriftPage({ limit: 200 });
  ...
  it("mixed registry+source drift: total_registry and total_sources correct across any limit", async () => {
    ...
    const p = await engine.detectDriftPage({ limit: 1, offset: 0 });
    ...
    const p2 = await engine.detectDriftPage({ limit: 5, offset: 15 });
  ...
  it("BOUNDED ARRAY: 60 drift entries + limit=10 → items.length===10 (NOT 60), totals accurate", async () => {
    ...
    const p = await engine.detectDriftPage({ limit: 10, offset: 0 });
    ...
    const pMid = await engine.detectDriftPage({ limit: 10, offset: 30 });
    ...
    const pLast = await engine.detectDriftPage({ limit: 10, offset: 55 });
    ...
    for (let off = 0; off < 60; off += 10) {
      const pg = await engine.detectDriftPage({ limit: 10, offset: off });
...
describe("reconcileDrift", async () => {
  ...
  it("dryRun (default true): source drift → action=propose, no proposal created", async () => {
    const env = makeEnv();
    registerWithSourceDrift("res:reconcile:a", env);

    const result = await engine.reconcileDrift(env, { rid: "res:reconcile:a" });
  ...
  it("dryRun:false — creates a pending proposal and does NOT commit (version unchanged)", async () => {
    const env = makeEnv();
    registerWithSourceDrift("res:reconcile:b", env);
    const before = engine.getResource("res:reconcile:b")!.current_version;

    const result = await engine.reconcileDrift(env, { rid: "res:reconcile:b", dryRun: false });
  ...
  it("FIX 1a: anonymous actor → reconcileDrift/propose throws, no proposals created", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(anonEnv, { rid: "res:reconcile:authz", dryRun: false });
  ...
  it("FIX 1a: propose() with empty actor_id → throws with descriptive error", async () => {
    const env = makeEnv();
  ...
  it("FIX 1a: propose() with actor_id='anonymous' → throws", async () => {
    const env = makeEnv();
  ...
  it("FIX 1a: propose() on critical resource → throws even with valid actor", async () => {
    const env = makeEnv();
  ...
  it("FIX 6: critical resource with source drift → action=skip (no surface), no proposal, no mutation", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(env, { rid: "res:critical:reconcile", dryRun: false });
  ...
  it("FIX 6: frozen resource missing source → action=skip (not surface)", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(env, { rid: "res:frozen:miss", dryRun: false });
  ...
  it("FIX 2: registry hash mismatch → surface + NO source proposal created (continue)", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(env, { rid: "res:registry:with-source", dryRun: false });
  ...
  it("registry content MISSING → surface + no source proposal", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(env, { rid: "res:registry:drift", dryRun: false });
  ...
  it("FIX 5: second reconcileDrift when proposal already pending → skip (dedup, no duplicate)", async () => {
    const env = makeEnv();
    registerWithSourceDrift("res:reconcile:dedup", env);

    await engine.reconcileDrift(env, { rid: "res:reconcile:dedup", dryRun: false });
    ...
    const result2 = await engine.reconcileDrift(env, { rid: "res:reconcile:dedup", dryRun: false });
  ...
  it("FIX 5: reconcileDrift dedup — second reconcile on same resource with pending proposal → skip (not duplicate)", async () => {
    ...
    const r1 = await engine.reconcileDrift(env, { rid: "res:uniqueidx:test", dryRun: false });
    ...
    const r2 = await engine.reconcileDrift(env, { rid: "res:uniqueidx:test", dryRun: false });
  ...
  it("FIX 3: N>limit sources → drift entries capped at limit, has_more=true", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(env, { limit: 5, offset: 0 });
    expect(result.planned.length).toBeLessThanOrEqual(5);
    // With offset=5: should get remaining entries.
    const result2 = await engine.reconcileDrift(env, { limit: 5, offset: 5 });
    expect(result2.planned.length).toBeLessThanOrEqual(5);
    // No single page exceeds limit.
    const bigResult = await engine.reconcileDrift(env, { limit: 200 });
    expect(bigResult.planned.length).toBeLessThanOrEqual(200);
  });

  // --- Never-commit invariant ---

  it("reconcileDrift(dryRun:false) never changes resource current_version", async () => {
    const env = makeEnv();
    ...
    await engine.reconcileDrift(env, { rid: "res:reconcile:nocommit", dryRun: false });
  ...
  it("explicit dryRun:true behaves same as omitted (no proposal created)", async () => {
    const env = makeEnv();
    registerWithSourceDrift("res:reconcile:explicit-dry", env);

    const result = await engine.reconcileDrift(env, { rid: "res:reconcile:explicit-dry", dryRun: true });
    expect(result.applied).toBe(false);
  ...
  it("FIX 3: reconcileDrift(dryRun:false, limit:N) creates exactly N proposals when >N drifts exist", async () => {
    const env = makeEnv();
    ...
    const result = await engine.reconcileDrift(env, { limit: 3, offset: 0, dryRun: false });
    ...
    const result2 = await engine.reconcileDrift(env, { limit: 3, offset: 3, dryRun: false });
...
describe("propose() UNIQUE index dedup (Fix 5 Round 3)", async () => {
  let baseDir: string;
  let engine: EvolutionEngine;
  let sql: SqliteStore;
  ...
  it("direct duplicate propose() throws PROPOSAL_ALREADY_PENDING (unique index enforcement)", async () => {
    const env = makeEnv();
  ...
  it("sequential propose→reject→propose: allowed (rejected row exits unique scope)", async () => {
    const env = makeEnv();
  ...
  it("propose() with unregistered actor throws", async () => {
    const env = makeEnv();
  ...
  it("propose() with registered actor succeeds", async () => {
    const env = makeEnv();
  ...
  it("migration dedup: pre-existing duplicate active proposals get 'superseded' on V7", async () => {
...
describe("getResource audit (Fix Round 3+)", async () => {
  let baseDir: string;
  let engine: EvolutionEngine;
  let sql: SqliteStore;
  ...
  it("getResource without env: returns resource, no audit event", async () => {
    const env = makeEnv();
  ...
  it("getResource with env: returns resource AND emits evolution.read audit event", async () => {
    const env = makeEnv();
  ...
  it("getResource with env for missing rid: returns null AND emits audit event with found=false", async () => {
    const env = makeEnv();
    const countBefore = (sql.db.prepare(`SELECT COUNT(*) as n FROM events WHERE kind = 'evolution.read'`).get() as { n: number }).n;
