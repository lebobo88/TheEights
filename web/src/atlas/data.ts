/* TheEights — Agent-BOM Atlas: the entire codebase as a graph.
   Verbatim typed port of design-reference/project/atlas/data.js.
   ...
   This is the curated STRUCTURE skeleton. It stays static; live observability is
   overlaid at runtime by useAtlasLive (see ../atlas/useAtlasLive.ts). */
...
const G: Record<string, GroupDef> = {
...
const L = (s: string, t: string, rel = ""): void => {
  links.push({ s, t, rel });
};
...
/* ---------- CONSUMERS ---------- */
const consumers: Array<[string, string, number, string, string]> = [
...
const nsDesc: Record<string, string> = {
...
/* ---------- COGNITIVE SERVICES ---------- */
const cog: Array<[string, string, string]> = [
...
/* ---------- STORES ---------- */
const stores: Array<[string, string, number, string, string]> = [
...
/* ---------- ADAPTERS ---------- */
const adapters: Array<[string, string, string, string]> = [
...
/* ---------- SCHEMAS ---------- */
const schemas: Array<[string, string, string]> = [
...
const memtypes: Array<[string, string, string]> = [
...
/* ---------- EIGHT CELLS ---------- */
(["vision", "context", "triggers", "influence", "risk", "focus", "constraints", "delight"] as const).forEach((c, i) => {
...
const bomNodes: Array<[string, string]> = [
...
/* ---------- RESOURCES (kinds) ---------- */
const reskinds: Array<[string, string, string, string]> = [
...
/* ---------- PROPOSALS / HITL ---------- */
N("hitl", "HITL Queue", "proposal", 11, "Evolution proposals awaiting operator-signed HITL approval. Non-low risk classes require explicit approval before commit.", { meta: "pending approval", path: "~/.eights/evolution/pending/" });
L("hitl", "eng-governance-state", "proposes");
const props: Array<[string, string, string, string, string]> = [
  // RLM Platform (10)
...
/* ---------- ADRs ---------- */
const adrs: Array<[string, string, string]> = [
...
(["pp", "hydra", "execsuite", "rlm"] as const).forEach((c) => {
...
] as Array<[string, string, string]>).forEach(([id, label, desc]) => {
...
] as Array<[string, string, string, string, string]>).forEach(([id, label, desc, path, link]) => {
...
] as Array<[string, string, string, string]>).forEach(([id, label, desc, path]) => {
...
] as Array<[string, string, string, string]>).forEach(([id, label, desc, link]) => {
...
export const ATLAS: AtlasData = { groups: G, nodes, links, lenses };
